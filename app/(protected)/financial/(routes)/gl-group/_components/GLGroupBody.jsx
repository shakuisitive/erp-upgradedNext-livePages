...
import GLGroupStatus from "./customComponents/GLGroupStatus";
...
import { closeDrawer } from "../_redux/GLGroupSlice";
import NewGroupCatagory from "./customComponents/NewGroupCatagory";
import EditGroupCatagory from "./customComponents/EditGroupCatagory";
import GLGroupActivity from "./customComponents/GLGroupActivity";
import GLGroupRightDrawer from "./customComponents/GLGroupRightDrawer";
...
import PayableAndRecievable from "./customComponents/DropdownPayAndRec"
import { onDataLoad } from "./../_redux/GLGroupSlice"
...
const ProductCategoryBody = () => {
  ...
  // const data = useSelector(store => store.glGroupSlice.products)
  // const inactiveData = useSelector(store => store.glGroupSlice.inActiveProducts)
  // const activeData = useSelector(store => store.glGroupSlice.activeProducts)
  const [colaps, setColaps] = useState(false);
  const [colapsComp, setColapsComp] = useState(false);
  const [checkedItems, setCheckedItems] = useState([]);
  const [checkedAll, setCheckedAll] = useState(false);
  const refreshes = useSelector((state) => state.glGroupSlice.refreshing);
  ...
  const [head, setHead] = useState([
    {
      ...
      Wid: 300,
      filter: "textFilter",
      customComp: GLGroupActivity,
      Modal: GLGroupRightDrawer,
      Drawer: GLGroupActivity,
    },
    {
      title: "Owner",
      slector: "",
    ...
    },
    {
      ...
      Wid: 90,
      customComp: PayableAndRecievable,
      filter: "checkFilter",
      checkFilterOptions: ["NEW"],
    },
    { title: "Status", slector: "", Wid: 150, Status: GLGroupStatus },
  ]);
  const filterTabs = {
    actionBtn: {
      // option: option,
    ...
    const gridArr = [
      {
        ...
        footerComp: {
          ...
          GriddFooterAdd: NewGroupCatagory,
        },
        checkBox: {
  ...
  useEffect(() => {
    let apiData = {
      PromotionApiData: [
        {
          api: Inventory.GetGlAccountGroup,
          payload: payload,
