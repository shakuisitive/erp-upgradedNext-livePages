...
import { UpdateUserId, readySubGridPayLoad, setAssignDrawer, setPhysicalCountDetails, setPhysicalCountForm, setRefresh } from "../redux/TransferSlice";
...
const AssignDrawer = () => {
  const userList = useSelector((state) => state.TransferSlice.userList);
  ...
  const getUser = userList.find(
    (item) => item.USE_ID == TransferForm?.USE_ID_PREPARED_BY
  );
  ...
  const onSubmit = () => {
    if(TransferForm.APPROVED_FLAG == "Y") {
        dispatch(readySubGridPayLoad({ id: TransferForm.PHYCOU_ID }));
        setIsSend(true);
    } else {
