...
import { UpdateUserId } from "../../redux/TransferSlice";

const AssignDropDown = () => {
  const userList = useSelector((state) => state.TransferSlice.userList);
  const TransferForm = useSelector((state) => state.TransferSlice.TransferForm[0]);
  const dispatch = useDispatch();
  const handleSelectedOptionChange = (option) => {
    dispatch(UpdateUserId(option.USE_ID));
  };

  const getUser = userList.find((item) => item.USE_ID == TransferForm?.USE_ID_PREPARED_BY)
