...
const PurchaseForm = ({pdfModal, setPdfModal, pdf, setPdf, }) => {
  ...
  const zeroVal = useSelector(
    (state) => state.PurchaseSlices.postIssueZeroState
  );
  ...
  const allFieldsFilledDetail = dataDetails.every((item) => {
    return item.QUANTITY > 0 && item.COST > 0 && item?.NET_COST > 0;
  });

  const handleSkipEmailIssuOrder = (val) => {
    if (updatedPostData?.length > 0) {
      if (zeroVal == false && allFieldsFilled && allFieldsFilledDetail) {
        dispatch(setIssueStatus());
        dispatch(clearVenderListFormData());
      } else {
        setEMessage(
          "Product fields Must be filled."
        );
  ...
  useEffect(() => {
    const handleKeyDown = (event) => {
      if (event.key === "F4" && FormStatus == "Initiated" ) {
