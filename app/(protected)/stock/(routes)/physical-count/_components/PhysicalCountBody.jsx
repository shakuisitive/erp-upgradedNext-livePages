...
import NewPhysicalCount from "./NewPhysicalCount/NewPhysicalCount";
import PCFormOpen from "./PCMainGrid/PCFormOpen";
import CustomModal from "./../../../../../../components/misc/pureComponents/custommodal/CustomModal";
import PhysicalCountForm from "./PhysicalCountForm/PhysicalCountForm";
...
import BinLocSelect from './PCSubGrid/BinLocSelect';
import SecLocSelect from './PCSubGrid/SecLocSelect';
import RowLocSelect from './PCSubGrid/RowLocSelect';
import PhysicalCountQTY from './PCSubGrid/PhysicalCountQTY';
import ItemSplit from './PCSubGrid/ItemSplit';
import LotNumber from './PCSubGrid/LotNumber';
import RightDrawer from "../../../../../../components/misc/pureComponents/rightdrawer/RightDrawer";
import PCMainAction from './PCMainAction';
import AssignDropDown from "./PhysicalCountForm/AssignDropDown";
import AssignDrawer from './AssignDrawer';

const PhysicalCountBody = () => {
  ...
  const verifyCodeModal = useSelector(
    (state) => state.physicalCount.verifyCodeModal
  );
  const isModalOpen = useSelector((state) => state.physicalCount.isModalOpen);
  const newModal = useSelector((state) => state.physicalCount.newModal);
  const newSModal = useSelector((state) => state.physicalCount.newSModal);
  let PCModal = useSelector((state) => state.physicalCount.PCModal);
  const loading = useSelector((state) => state.physicalCount.mainLoader);
  const refresh = useSelector((state) => state.physicalCount.refresh);
  let PCIndex = useSelector((state) => state.physicalCount.PCIndex);
  const CsvData = useSelector((state) => state.physicalCount.CsvData);
  const sessionId = useSelector((state) => state.physicalCount.sessionId);
  const subData = useSelector((state) => state.physicalCount.subData);
  const assignDrawer = useSelector((state) => state.physicalCount.assignDrawer);
  // useEffect(() => {
  //   if (refresh == true) {
  //     setData([]);
  //     setInactiveData([])
  ...
  const ToggleNewSModal = (set) => {
    dispatch(setPhysicalCountDetails([]));
    dispatch(setSelectedWarid({}));
    dispatch(setNewForm());
    dispatch(setNewSModal(set));
  };
  ...
  const handleVerifyNonModal = () => {
    // dispatch(setPhysicalCountDetails([]))
    ToggleNewSModal(true);
    setIsNonModalOpen(false);
  };

  const option = [
    {
      label: "New non-Stock PC",
      icon: IoIosAdd,
      onClick: () => setIsNonModalOpen(true),
    },
  ];
  ...
  const [payloadBranchA, setPayloadBranchA] = useState({
    data: {
      ...
      LOT_NUMBER: "",
      LOT_EXPIRY_DATE: "",
      PHYSICAL_COUNT_STATUS: "Initiated",
      WAR_ID: "",
  ...
  const [payloadBranch, setPayloadBranch] = useState({
    data: {
      ...
      LOT_NUMBER: "",
      LOT_EXPIRY_DATE: "",
      PHYSICAL_COUNT_STATUS: "Complete",
      WAR_ID: "",
      NON_STOCK_ITEMS_FLAG: "",
    },
    action: "InventoryWeb",
  ...
  const handleBranchA = (data) => {
    setData(data.Result);
    dispatch(setPCList(data.Result));
    dispatch(setLoader(false));
  };
  const handleBranch = (data) => {
    const items = data.Result.filter((item) => item.PC_STATUS == "Complete" || item.PC_STATUS == "Void")
  ...
  const tabsN = [
    {
      icon: <GoHome />,
      label: "Details",
      content: <NewPhysicalCount nonSPC={true} />,
    },
  ];
  ...
  return (
    <div className="w-full  h-fit  flex flex-col overflow-auto pb-5">
      ...
      {isNonModalOpen && (
        <VerifyModal
          onClose={() => setIsNonModalOpen(false)}
          cancle={"cancel"}
          verify={"Verify"}
          msg="Continuing with the inventory count while sales and transfers are in progress might result in discrepancies. Do you wish to proceed ?"
          action={handleVerifyNonModal}
        />
      )}
      {isModalOpen && (
        <VerifyModal
...
export default PhysicalCountBody;
