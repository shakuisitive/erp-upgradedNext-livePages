...
const FormStockGridSku = ({ data, rowData, index, }) => {
  const dispatch = useDispatch();
  let [error, sendRequest] = useApiFetch();
  const [isDrawer, setIsDrawer] = useState(false);
  const obj = useSelector((state) => state.stockSlices.subData[0]);
  ...
  const openModal = () => {
    dispatch(
      setOrderIds({ orderId: obj?.id, detailId: rowData.INVSTODET_ID })
    );
...
export default FormStockGridSku;
