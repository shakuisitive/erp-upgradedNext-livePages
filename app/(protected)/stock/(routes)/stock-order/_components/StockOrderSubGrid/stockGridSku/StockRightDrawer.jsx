...
import { loaderToggle, readySubGridPayLoad, setRefresh, updateSubGridData } from "../../../redux/stockSlice";

const StockRightDrawer = ({ id, rowId }) => {
  ...
  const product = useSelector((state) => state.stockSlices.stockOrderDetailData[0]);
  ...
  const onSubmit = () => {
    ...
    let currLocation = LocationsSet.find(
      (item) =>
        item?.SECTION == product.RACK &&
        item?.ROW == product.SHELF &&
        item?.BIN == product.BIN
    );
    locId = currLocation?.WARSTOLOC_ID;
    dispatch(updateSubGridData({SOId: stockOrderFormDataId, SPId: product.INVSTODET_ID, locId: locId}))
          ...
          <div className="w-1/2 ml-2">
            <div className="flex items-center gap-3">
              ...
              <div className="bg-white border border-gray-300 rounded w-full p-1">
                <span className="text-gray-400 text-customblack text-[14px]">{product.INVENTORY}</span>
              </div>
            </div>
          </div>
          <div className="w-1/2">
