...
import { UpdateUserId, readySubGridPayLoad, setAssignDrawer, setPhysicalCountDetails, setPhysicalCountForm, setRefresh } from "../redux/CycleCountSlice";
...
const AssignDrawer = () => {
  const userList = useSelector((state) => state.CycleCountSlice.userList);
  ...
  const getUser = userList.find(
    (item) => item.USE_ID == CycleCountForm?.USE_ID_PREPARED_BY
  );
  ...
  const onSubmit = () => {
    if(CycleCountForm.APPROVED_FLAG == "Y") {
        dispatch(readySubGridPayLoad({ id: CycleCountForm.PHYCOU_ID }));
        setIsSend(true);
    } else {
