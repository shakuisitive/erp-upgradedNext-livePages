...
const CycleCountMainAction = ({ rowData, data, obj }) => {
  ...
  const postPayload = useSelector((state) => state.CycleCountSlice.subPayload);
  ...
  const getPdfData = (data) => {
    if (data?.CODE == "SUCCESS") {
      if (data.bytes) {
        ...
        setPdf({ title: "Cycle Count Report", pdfUrl: url });
        setPdfModal(true);
      } else {
...
export default CycleCountMainAction;
