...
import CycleCountQTY from "./CycleCountQTY";
import PhysicalCountFormHeader from "./CycleCountFormHeader";
import {
  Administration,
  PhysicalCount,
...
import PCAdjustment from "./CycleCountAdjustment";
...
import PhysicalCountForm from './../../../physical-count/_components/PhysicalCountForm/PhysicalCountForm';
...
const CycleCountForm = () => {
  ...
  let CycleCountForm = useSelector(
  ...
  const [hideOnhandQty, setHideOnhandQty] = useState(CycleCountForm?.ONHAND_BLANK_FLAG == "Y" ? true : false);
    ...
    if (data.CODE === "SUCCESS") {
      const physicalDetail = data.Result.map((items) => {
        return {
          ...
          PHYCOU_ID: CycleCountForm.PHYCOU_ID,
        };
      });
  ...
  const generateList = () => {
    sendRequest(
      CycleCountSlice.GetAutoPCDetailList,
      "POST",
      listPayload,
      getList,
      accessToken
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="flex flex-col justify-end">
              <h2 className="text-customblack text-[24px] leading-[24px] font-normal ">
                {CycleCountForm?.PC_NUMBER}
              </h2>
              <p className="text-[#6b7280] text-[14px] leading-[24px] font-normal text-right">
                {moment(CycleCountForm?.PC_DATE).format("DD / MM / YYYY")}
              </p>
            </div>
          </div>
