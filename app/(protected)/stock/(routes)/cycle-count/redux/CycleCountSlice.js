...
const CycleCountSlice = createSlice({
  ...
  reducers: {
    ...
    setPhysicalCountForm(state, action) {
      state.CycleCountForm = action.payload;
    },
    setPhysicalCountDetails(state, action) {
      state.physicalCountDetails = action.payload;
    },
    addCycleCountDetails(state, action) {
      const combine = [...state.physicalCountDetails, ...action.payload];
      return {
        ...state,
        physicalCountDetails: combine,
      };
    },
    onDeleteRow(state, action) {
      const updatedItems = [...state.physicalCountDetails];
      updatedItems.splice(action.payload.index, 1);
      return {
        ...state,
        physicalCountDetails: updatedItems,
      };
    },
    ...
    updateVoid(state, action) {
      state.CycleCountForm = state.CycleCountForm.map((item) => {
        item.VOID_FLAG = "Y";
    ...
    readySubGridPayLoad(state, action) {
      const physicalForm = state.CycleCountForm.map((items) => {
        const {
          APPROVED_FLAG,
          CYCLE_COUNT_FLAG,
          FINILIZE_TYPE,
      ...
      const physicalDetail = state.physicalCountDetails.map((items) => {
        ...
        return {
          ...
          USE_ID: "2694",
          WARSTOLOC_ID,
          WAR_ID: state.CycleCountForm[0].WAR_ID,
        };
      });
...
export default CycleCountSlice;
export const {
  setNewModal,
  setPCIndex,
  setPCModal,
