...
const AddUpSale = ({ title, id }) => {
    ...
    data: [
      {
        ...
        CHILD_PAR_ID: upSale,
        ACTIVE_FLAG: "Y",
        PARENT_PAR_ID: formIndex?.PAR_ID,
      },
    ],
    action: "Administration",
  ...
  useEffect(() => {
    if (isSend == true) {
      // console.log("upsale: ",getDetailPayload)
      sendRequest(
  ...
  const availableTaxList = getAvailableOptions(partList);
  return (
    <div className="m-2 ">
      <Dropdown
        ...
        inputClassName="w-fit focus:outline-none hover:bg-transparent border border-transparent hover:border-gray-200 text-customblack text-[14px] leading-[21px] font-normal"
