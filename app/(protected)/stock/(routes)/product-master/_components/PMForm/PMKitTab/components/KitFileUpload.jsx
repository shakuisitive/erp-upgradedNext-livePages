...
import { deleteAttachment, setKitImagesData } from "../../../../redux/pmSlice";

const KitFileUpload = ({ onUpload }) => {
  ...
  const imagesData = useSelector((state) => state.pmSlices.KitImagesData);
  const formIndex = useSelector((state) => state.pmSlices.kitFormIndex);

  const partAttachmentPayload = {
    data: {
      KIT_ID: formIndex?.KIT_ID.toString(),
      SEARCH: "",
    },
    action: "ITEMMASTER",
  ...
  useEffect(() => {
    if (imagesData?.length > 0) {
      imagesData?.forEach((item) => {
        const baseUrl = `${process.env.NEXT_PUBLIC_REACT_APP_API_BASE_URL}ITEMMASTER/GetAttachmentFile?ModuleName=KIT&AttachName=${item?.ATTACH_NAME}`;
        fetch(baseUrl, {
          method: "GET",
          body: null,
