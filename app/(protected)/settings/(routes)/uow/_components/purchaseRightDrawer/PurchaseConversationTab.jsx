...
import {setChatData,setPostLikes} from '../../redux/Purchase.slice';
...
//conversation
let data1=useSelector((state)=>state.PurchaseSlices.editorData); 
let chatData=useSelector((state)=>state.PurchaseSlices.ChatData);
...
let username=useSelector((state)=>state.PurchaseSlices.username); 
let userId=useSelector((state)=>state.PurchaseSlices.user_ID); 
...
return (
 <div className="flex flex-col gap-10  bg-white overflow-auto  h-[85vh]  ">
                    ...
                    <div className='text-right hover:text-blue-500 mt-0 mb-0'> Write updates via email</div>
                     <div className=' p-6 max-w-sm mx-auto mt-20 items-center h-screen justify-content'>
                      <div className='pl-16 item-center'>
                        <div className='justify-center items-center pl-26 '>
                        <img className='h-100 w-100' src='https://cdn.monday.com/images/pulse-page-empty-state.svg'alt='pulse-page-e'/>
                        </div>
                      </div>
                      <div><h1 className=' text-[26px] text-black pl-16 mt-6 justify-content:center text-2xl text-nowrap font-semiblod '>
                               No updates yet for this item</h1>
                        <p className='text-nowrap mt-4 text-base' >
                        Be the first one to update about progress, mention someone 
                        <br/>
                      <span  className=' p-10 justify-content: center'>  or upload files to share with your team members</span>
                        </p>
                      </div>
                     </div>
