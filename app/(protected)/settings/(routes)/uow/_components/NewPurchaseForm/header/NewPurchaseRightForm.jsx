...
import { NewPurchaseNotes , NewPurchaseRef,updatePurchaseNotes } from '../../../redux/Purchase.slice'


const NewPurchaseRightForm = () => {
  ...
  // const PurchaseDetails = useSelector((state) => state.PurchaseSlices.postPurchaseOrder)

  // // console.log("checking purchase madiha detail",PurchaseDetails)

   const handleInputChangeRef = (e) => {
    setRef(e.target.value);
    dispatch(NewPurchaseRef(e.target.value))
  };
const handleInputChangeNotes = (e) =>{
    setNotes(e.target.value)
    dispatch(NewPurchaseNotes(e.target.value))
  }
    ...
    <div className="flex gap-10 flex-col md:flex-row items-start w-full h-full bg-[#e1eff2] rounded-[6px] shadow-md shadow-blue-50 border border-customgreen px-6  py-3">
      <div className="  w-full md:w-1/2 flex flex-col gap-3 ">
        ...
        <div className="flex gap-1 ml-2 mt-2">
          ...
          <textarea type="text" className="lgdesktop:w-[87%] desktop:w-[75%] laptop:w-[70%] tablet:w-[70%] h-[100px] focus:outline-none border focus:border-customGray bg-white overflow-auto" />
        </div>
      ...
      <div className="   w-full  md:w-1/2  flex flex-col gap-3  mt-[2px]">
        <div className="flex gap-2 ml-2 mt-2">
          <p className="text-[14px] text-customblack focus:outline-customblack">Ref#</p>
          <input onChange={handleInputChangeRef} disabled={false} value={ref}  type="text" className="w-full bg-white focus:outline-none border focus:border-customGray" />
        </div>
        <div className="flex gap-1 ml-2 mt-4">
          <p className="text-[14px] text-customblack focus:outline-customblack">Notes</p>
          <textarea onChange={handleInputChangeNotes} disabled={false} value ={notes}  type="text" className="w-full h-[100px] bg-white focus:outline-none border focus:border-customGray" />
        </div>
...
export default NewPurchaseRightForm;
