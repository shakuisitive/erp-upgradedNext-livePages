...
import { updatePurchaseNotes ,updatePurchaseRef, closeModallForm ,  subGridset , setPurchaseDetails , setUpdatePurchaseDetail , setUpdatePurchaseOrder , setLotList , setProductOrederUpdate , setIssueStatus , setReadyForRStatus , setcloseModall , setRefresh , onHeadVis} from '../redux/Purchase.slice'
...
useEffect(()=>{
  const hasZeroCount = dataDetails.some(item => parseInt(item.QUANTITY) === 0);
...
if(zero == false){
  ...
  useEffect(()=>{
    if(data[0]?.APPROVED_FLAG == 'Y' && data[0]?.COMPLETE_FLAG  == 'N'){
      sendRequest(apiUrlPOrder, 'POST', payloadPOrder, getAllTaskPOrder, token , "issue")
  ...
  useEffect(()=>{
    if(data[0]?.APPROVED_FLAG == 'Y' && data[0]?.COMPLETE_FLAG  == 'Y'){
      sendRequest(apiUrlPOrder, 'POST', payloadPOrder, getAllTaskR, token , "ready")
      // sendRequest(apiUrlDetails, 'POST', payloadDetails, getProdectDetailResR, token)
      // sendRequest(apiUrlR , 'POST', payloadRPO, getProdectDetailResPO, token)
  ...
  return (
    ...
    <div className='  h-[98%] mt-[4px] gap-2   flex     rounded-lg'>
        ...
        <div className={`  flex flex-col relative  border ${formColaps == true ? "grow" : "lgdesktop:w-[75%]   desktop:w-[70%] laptop:w-[60%] tablet:w-[50%]"}   rounded-md bg-white`} >
...
<div className={`absolute bottom-0 left-0 mb-8 ml-8 ${headVis == true ? "block" : "hidden"} `}>
  <VenderDetails/>
        ...
        <div className={`px-4 border ${formColaps == true ? " w-[0px] overflow-hidde" : "lgdesktop:w-[25%] desktop:w-[30%]  laptop:w-[40%] tablet:w-[50%] "}  bg-white transition-all  duration-300    rounded-md shadow-md shadow-gray-200 py-5`}  >
          <div className={`flex items-center justify-between `}>
            ...
            <div className={` ${formColaps == true ? "hidden" : "block" } delay-700 `}>
              <p className='H text-gray-800 lgdesktop:text-[30px] btdesktop:text-[30px] text-[25px]'>{formData?.PO_NUMBER}</p>
              <p className='H text-gray-500 lgdesktop:text-[18px] btdesktop:text-[18px] text-[16px]  text-right '>{date}</p>

            </div>
          </div>
          <div >
            <div onClick={()=>setFormColaps(!formColaps)} className='py-2   rounded-r-md cursor-pointer w-fit border -translate-x-6 bg-white'>
            <IoIosArrowForward className=' text-[18px]    '/>
         </div>
          </div>
      
          <div className={`w-full mt-4  justify-center  ${formColaps == true ? "hidden" : "flex" } `}>
            
              <button onClick={postIssue} className={`w-full max-w-[430px]  bg-customgreen hover:bg-btnHoverGreen text-white flex justify-center items-center font-semibold py-2 rounded-full ${FormStatus == 'Initiated' ? 'block' : 'hidden'}`}>Issue to Vendor</button>
              <button onClick={postReadyForReceaving} className={`w-full max-w-[430px]  bg-customgreen hover:bg-btnHoverGreen text-white flex justify-center items-center font-semibold py-2 rounded-full ${FormStatus == 'Issued to Vendor' ? 'block' : 'hidden'}`}>Ready for Receiving</button>
            
          </div>
          <div className={`w-full mt-2  justify-center ${formColaps == true ? "hidden" : "flex" } `}>
          ...
          <InputTextEut label="Ref" placeHolder='Ref' initialValue={formData?.REFERENCE_NUMBER} value={inputValue} onChange={handleInputChangeRef} isDisabled={false} />
