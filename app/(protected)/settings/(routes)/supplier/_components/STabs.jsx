...
import { setEditHead, setHeadReduxT,setSearchData ,setIsHitApi,OpenNewModalForm,setRefresh} from "../redux/supplierSlice";
...
const [foc , setFoc] = useState(false)
  const paymentHead = useSelector((state) => state.supplier.paymentHead);
  const defHead = useSelector((state) => state.supplier.defHead);
  const skuList = useSelector((state) => state.supplier.skuList);
  const paymentMainGrid = useSelector((state) => state.supplier.supplierMainGrid);
  const username=useSelector(state=>state.supplier.username);
