...
import { setEditHead, setHeadReduxT,setSearchData ,setIsHitApi,OpenNewModalForm,setRefresh} from "../_redux/paymentTermSlice";
...
const [foc , setFoc] = useState(false)
  const paymentHead = useSelector((state) => state.paymentTerm.paymentHead);
  const defHead = useSelector((state) => state.paymentTerm.defHead);
  const skuList = useSelector((state) => state.paymentTerm.skuList);
  const paymentMainGrid = useSelector((state) => state.paymentTerm.paymentMainGrid);
  const username=useSelector(state=>state.paymentTerm.username);
  
  console.log(paymentHead);
  console.log(paymentMainGrid)
  
  const checkUpdatelist = useSelector(
    (state) => state.PurchaseSlices.VenderList
  );
//   console.log('check head redux' , paymentHead);
...
const dispatch = useDispatch()
  ...
  const setFilter = (e) => {
    const fil = paymentHead.filter((data) => data.title.toLowerCase().includes(e.target.value.toLowerCase()));
    setFHead(fil);
  ...
  const setFilterPo = (e) =>{
    ...
    const fil = paymentMainGrid.filter((data) => data.PO_NUMBER.toLowerCase().includes(e.target.value.toLowerCase()));
    setPoListS(fil);
    console.log('check out filter', e.target.value, fil);
  }
...
const check = paymentHead.findIndex((data)=> data.title == titleP)
...
console.log('check index in tabs' , check);
  ...
  useEffect(() => {
    if (paymentMainGrid.length > 0) {
      // Check if arr is not empty
      const sortedArr = [...paymentMainGrid]; // Create a copy of arr
      sortedArr.sort((a, b) => {
...
let isHitApi=useSelector((state) => state.paymentTerm.isHitApi);
let refresh=useSelector((state) => state.paymentTerm.Refresh);

const debounce=useDebounce(search,1000);
let [error, sendRequest] = useApiFetch();

let searchURL=`${process.env.NEXT_PUBLIC_REACT_APP_API_BASE_URL}Administration/GetPaymentTermList`;
...
<div className="  flex justify-between items-center text-customblack hover:bg-slate-100 mt-5 px-3 rounded-lg border ">
<Dropdown  options={paymentMainGrid} optionKey1="PO_NUMBER" optionKey2="PO_NUMBER" onSelectedOptionChanged ={setChangeP} placeholder="select Po Number" inputClassName="focus:outline-none w-[270px] hover:bg-transparent  py-2 
        hover:border-gray-200 text-customblack text-[14px] leading-[21px] font-normal" dropdownClassName="bg-white w-full border border-customGray rounded-[4px] mt-1 text-customblack shadow-md shadow-gray-400 "  customFocusKey = "" isDisabled={false }
        onClearInputValue={false} onHandleFocus={handleOnFocus} onDefaultInput = '' onHandleBlur={handleOnBlur} forwardedRef={dropdownRef}
        />
        <IoIosArrowDown/>
