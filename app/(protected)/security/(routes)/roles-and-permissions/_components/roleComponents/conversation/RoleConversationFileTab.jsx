...
import { addFile } from "../../../redux/Role.slice";


const RoleConversationFileTab = ({ data }) => {
  ...
  const listView = useSelector((state) => state.RoleSlice.files).filter(
    (item) => item.roleName == name
  );
  const GridView = useSelector((state) => state.RoleSlice.files).filter(
    (item) => item.roleName == name
  );

  console.log(listView, "list view")
  ...
  const files = useSelector((state) => state.RoleSlice.files);
...
export default RoleConversationFileTab;
