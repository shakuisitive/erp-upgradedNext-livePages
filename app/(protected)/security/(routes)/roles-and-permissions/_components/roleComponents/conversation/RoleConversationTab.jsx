...
import { setChatData, setPostLikes } from "../../../redux/Role.slice";
...
import {
  getCurrentDateTime,
  timeAgo,
} from "./../../../../../../../../components/misc/pureComponents/rightdrawer/Functions";

const RoleConversationTab = ({ data }) => {
  ...
  //conversation
  let data1 = useSelector((state) => state.RoleSlice.editorData);
  let chatData = useSelector((state) => state.RoleSlice.ChatData);

  let autgro_id= data?.AUTGRO_ID;
  let newData = chatData
    .filter((item) => item.autgro_id == autgro_id)
    .sort((a, b) => b.id - a.id);
    ...
    <div className="flex flex-col gap-10  bg-white overflow-auto  h-[85vh]  ">
      {isEditor ? (
        ...
        >
          <div
            style={{
              width: inputAnim ? "95%" : "100%",
              height: inputAnim ? "95%" : "100%",
            }}
            className="text-[16px] px-[20px] hover:bg-[#6768791a] cursor-pointer  py-[10px] border border-[#0073ea] rounded-[8px] mt-[30px] text-customblack"
          >
            Write an update...
          </div>
        </div>
      ...
      {isEditor && (
        <div className="mt-[20px] flex flex-col gap-1">
          <QuillEditor toolid={"drawerHome"} func={editorHandler} />
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="bg-[#0073ea] focus:bg-[#3b79bb] text-white px-2 p-1 rounded-[4px]"
            >
              Update
            </button>
          </div>
      ...
      {newData &&
        newData.map((data, index) => (
          <div key={index} className="border rounded-[8px] ">
            ...
            <p className="p-3 min-h-[80px]">
              <Message string={data.message} />
            </p>
            <div className="h-[40px] flex border-t border-[#d0d4e4]">
              ...
              <button className="flex items-center gap-1 justify-center flex-[0.5] ">
                <div>
...
export default RoleConversationTab;
